...
标题：无限网格补单不及时次数统计（基于价格触线推断）

目标：在动态网格运行过程中，当价格（mid）触达应挂单价位且该价位挂单缺失时，计为一次补单不及时。每个价位缺单仅计一次，待挂单恢复或该价位不再缺单后可再次计数。仅动态网格启用，AS 网格不计数。bot.start 时重置。

实现要点：
- BotStatus 增加字段 `delay_count`（补单延迟次数）
- `/api/runtime/status` 的 `symbols[*].delay_count` 输出补单延迟次数
- WebUI 运行状态表新增列“补单延迟”，展示 `delay_count`

计数规则（默认）：
- 触线判定：ask 侧 `mid >= price`，bid 侧 `mid <= price`
- 无容错区间，不做价格偏移
- 同一缺单价位只计一次，恢复后再缺单可重新计数
