...
- WebUI：管理员密码、Lighter 配置、策略配置、启动/停止、紧急停止（撤网格单）、日志
...
- `apps/server/requirements.txt` 会通过 `git+https://...` 拉取官方 `lighter-python`
...
2) 交易所配置
   - 选择 mainnet/testnet
   - 填写 L1 地址、account_index、api_key_index、API 私钥（可选 ETH 私钥）
   - 可点“自动查询 account_index”
3) 拉取市场列表，找到你要跑的 BTC/ETH/SOL perp 的 `market_id`
4) 策略配置
