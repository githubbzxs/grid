...
- [ ] 验证 market_id 后台自动匹配与前端隐藏展示是否符合预期
- [ ] 验证 GRVT 启动时 market_id 自动解析是否生效
...
- **[2026-02-05]**: WebUI 调整布局（移除市场列表按钮，运行统计合并历史，账户/挂单合并日志，交易所配置移至倒数第二），策略新增交易所字段并按当前交易所过滤启动。
- **[2026-02-05]**: 本机服务重新部署并确认 127.0.0.1:9999 可访问。
- **[2026-02-05]**: Lighter 运行统计/历史的 PnL 改为成交口径总 PnL，并为成交查询补充 auth 避免 400 缺少授权。
- **[2026-02-05]**: Lighter 策略 PnL 统计改为基于成交记录增量计算（逐笔更新持仓与已实现收益），并记录更新日志。
- **[2026-02-05]**: 为 Lighter 增加逐请求延迟诊断日志，并在补单阶段输出汇总日志（定位限速/重试/串行造成的慢）。
- **[2026-02-05]**: 取消密钥输入的状态胶囊提示，保存后保留密码输入的圆点显示（不再清空密钥输入框）。
- **[2026-02-05]**: market_id 改为后台自动匹配，前端不再显示相关列与输入；策略保存时自动补齐 market_id。
- **[2026-02-05]**: BotManager 在运行时缺失 market_id 时自动拉取市场并补齐（含 GRVT）。
...
- [2026-02-05 22:30] market_id 后台自动匹配与前端隐藏
  - 状态: 已完成
  - 变更: 后端保存策略时自动补齐 market_id；前端去除 market_id 列显示
  - 影响: 保存策略时将尝试拉取市场列表以匹配 market_id
- [2026-02-05 22:45] GRVT 启动时 market_id 自动解析
  - 状态: 已完成
  - 变更: BotManager 缺失 market_id 时自动解析并写回配置
  - 影响: GRVT 启动不再因 market_id 缺失立即报错
- [2026-02-05 21:12] 策略整体 PnL 统计问题澄清
  - 状态: 等待用户确认 PnL 口径与范围
  - 关注点: 现有统计是否为“本次运行增量”还是“当前总PnL”
  - 待确认: 统计范围、平仓后是否保留已实现PnL、手续费口径
- [2026-02-05 22:10] WebUI 结构与导航顺序梳理
  - 状态: 已完成结构与导航顺序定位
  - 关注点: 需求 2-4 的具体范围与验收点尚未给出
  - 待确认: 需求 2-4 的原始文本或编号对应的功能清单


- [2026-02-05 21:41] 策略配置/币对行新增“交易所”字段调研
  - 状态：已定位 WebUI 行生成与策略保存/读取链路，待确认字段默认值与用途
  - 关注点：币对级别交易所是否覆盖全局 exchange；是否允许混合交易所
  - 待确认：默认值来源（当前全局选择 / 固定值 / 手动选择）

- [2026-02-05 21:40] 查找拉取市场列表入口与按钮
  - 目标: 确认 WebUI/后端入口 index.html/app.js/main.py
  - 结果: 定位 WebUI 按钮 ID、JS 触发与 API 路由
  - 变更: 本次无代码变更
