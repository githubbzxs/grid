...
- [x] 服务器启用 systemd 服务 grid.service，保持 9999 端口常驻
- [x] 优化日志与挂单显示：输出改为可滚动预格式块，支持复制/清空与日志自动滚动开关
- [x] Lighter API 增加限流与 429 退避重试，缓解启动即 429
- [x] 网格补单优先级调整为优先内部档位，顶部/底部延后补单

**下一步计划**:
- [ ] 观察 Lighter 启动限流效果，必要时调大最小间隔与重试间隔
...
- **[2026-02-01]**: Lighter 交易所调用增加限流与 429 退避重试，降低启动限速报错。
